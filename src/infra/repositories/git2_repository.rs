...
impl GitRepository for Git2Repository {
    fn get_commits_not_in_branch(&self, branch: &BranchName) -> Result<Vec<Commit>, GitRepositoryError> {
        let head = self.repo.head().map_err(|e| GitRepositoryError::GitOperationFailed {
            message: format!("Failed to get HEAD: {}", e),
        })?;
        let head_commit = head.peel_to_commit().map_err(|e| GitRepositoryError::GitOperationFailed {
        ...
        let branch_commit = branch_ref.get().peel_to_commit().map_err(|e| GitRepositoryError::GitOperationFailed {
            message: format!("Failed to get branch commit: {}", e),
        })?;
        
        let commits_in_branch = self.get_all_commits(branch_commit.id())?;
        
        let mut revwalk = self.repo.revwalk().map_err(|e| GitRepositoryError::GitOperationFailed {
            message: format!("Failed to create revwalk: {}", e),
        })?;
        revwalk.push(head_commit.id()).map_err(|e| GitRepositoryError::GitOperationFailed {
        ...
        for oid in revwalk {
            ...
            if !commits_in_branch.contains(&oid) {
                let commit = self.repo.find_commit(oid).map_err(|e| GitRepositoryError::GitOperationFailed {
                    message: format!("Failed to find commit: {}", e),
                })?;
                let author = commit.author();
                
                let hash = CommitHash::new(oid.to_string()).map_err(|e| GitRepositoryError::GitOperationFailed {
    ...
    fn get_file_changes_between_branches(&self, branch: &BranchName) -> Result<Vec<FileChange>, GitRepositoryError> {
        ...
        let head_tree = head_commit.tree().map_err(|e| GitRepositoryError::GitOperationFailed {
        ...
        let branch_tree = branch_commit.tree().map_err(|e| GitRepositoryError::GitOperationFailed {
        ...
        let diff = self
            .repo
            .diff_tree_to_tree(Some(&branch_tree), Some(&head_tree), Some(&mut diff_options))
...
impl Git2Repository {
    fn get_all_commits(&self, start_oid: Oid) -> Result<HashSet<Oid>, GitRepositoryError> {
        ...
        revwalk.push(start_oid).map_err(|e| GitRepositoryError::GitOperationFailed {
