...
impl TestGitRepo {
    ...
    pub fn create_commit_on_current_branch(
        &self,
        message: &str,
        file_changes: &[(&str, &str)],
...
mod integration_tests {
    ...
    fn test_commit_comparison_service() {
        ...
        let git_repo =
            GitRepositoryImpl::open(test_repo.temp_dir.path().to_str().unwrap()).unwrap();
    ...
    fn test_git2_repository_get_file_changes_between_branches() {
        ...
        if file_changes.is_empty() {
            ...
            assert!(
                ...
                "Expected to find changes in new_file.txt or README.md"
            );
        }
    }
